...
def harris(im, sigma=1.0, rel_thresh=0.01, k=0.04):
    # had to change to float from np.float to fix an error given at run
...
for i1 in range(npts1):
    for i2 in range(npts2):
        ...
        mean1 = np.mean(patches1[:,:,i1])
        mean2 = np.mean(patches2[:,:,i2])
        # Calculate the cross-correlation between the patches
        cross_corr = np.sum((patches1[:,:,i1] - np.mean(patches1[:,:,i1])) * (patches2[:,:,i2] - mean2))
        # Calculate the normalization terms for NCC
        norm = np.sqrt(np.sum((patches1[:,:,i1] - mean1)**2)*np.sum((patches2[:,:,i2] - mean2)**2))
        # Calculate the NCC value
        distmat_ncc[i1, i2] = cross_corr / norm

# Find mutually nearest neighbors by maximizing NCC
ncc_ss1 = np.amax(distmat_ncc, axis=1)
ncc_ids1 = np.argmax(distmat_ncc, axis=1)
ncc_ss2 = np.amax(distmat_ncc, axis=0)
ncc_ids2 = np.argmax(distmat_ncc, axis=0)

pairs = []
for k in range(npts1):
    if k == ncc_ids2[ncc_ids1[k]]:
        pairs.append(np.array([k, ncc_ids1[k], ncc_ss1[k]]))
pairs = np.array(pairs)

# Sort the mutually nearest neighbors based on NCC in descending order
